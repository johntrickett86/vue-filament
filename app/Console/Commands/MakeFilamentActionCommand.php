...
class MakeFilamentActionCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        $path = $this->getFilePath($domain, $className, $type);
    ...
    private function getActionType(): string
    {
        ...
        if (count($selectedTypes) > 1) {
            ...
            throw new \LogicException('Please specify only one action type.');
        }
    ...
    private function getTargetDomain(): ?string
    {
        // Check if `app/Domain` directory exists
        if (! File::exists(app_path('Domain'))) {
            return null;
        }

        // Scan for available domain folders
        $domainFolders = collect(File::directories(app_path('Domain')))
            ->map(fn ($path) => basename($path))
            ->values() // Ensure numeric keys are reset
            ->toArray();
        ...
        // Add "None" option to allow skipping domain selection
        $domainFolders = array_merge(['None' => 'None'], $domainFolders);

        $selectedDomain = (string) select(
            label: 'Select the domain for this Filament Action (or select "None" to skip)',
            options: $domainFolders
        );

        return $selectedDomain !== 'None' ? $selectedDomain : null;
    }

    private function getFilePath(?string $domain, string $className, string $type): string
    {
        ...
        if ($domain) {
            return app_path("Domain/$domain/Filament/Actions/$subDirectory/$className.php");
        }
    ...
    {
        $stubPath = app_path('Filament/Support/Stubs/FilamentAction.stub');
